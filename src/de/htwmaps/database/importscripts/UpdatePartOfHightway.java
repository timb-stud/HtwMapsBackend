...
public class UpdatePartOfHightway {
	...
	private void start() {
		...
		try {
			PreparedStatement ps1 = sql.con.prepareStatement("UPDATE nodes SET partOfHighway = 1 WHERE ID IN (SELECT fromNodeID FROM edges WHERE partOfHighway = 1)");
			PreparedStatement ps2 = sql.con.prepareStatement("UPDATE nodes SET partOfHighway = 1 WHERE ID IN (SELECT toNodeID FROM edges WHERE partOfHighway = 1)");
			PreparedStatement ps3 = sql.con.prepareStatement("UPDATE edges SET partOfHighway = 1 WHERE wayID IN (SELECT r_way_tag.wayID FROM r_way_tag, k_tags WHERE k_tags.ID = r_way_tag.tagID AND k_tags.key = 'highway' AND k_tags.value IN ('motorway','motorway_link','trunk','trunk_link','primary','primary_link','secondary','secondary_link','tertiary','unclassified','road','residential','living_street'))");
			ps1.executeUpdate();
			System.out.println("sdfgjkghj,kdfghdjkfghgjhfjkghdfjkghdfgjkdfhgjkdfhg");
			ps2.executeUpdate();
			ps3.executeUpdate();
