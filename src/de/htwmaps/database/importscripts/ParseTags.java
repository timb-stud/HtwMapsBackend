...
public class ParseTags {
    ...
    private void start() {
		...
		try {
	        PreparedStatement ps1 = sql.con.prepareStatement("SELECT ID FROM `k_tags` WHERE `key` = ? AND `value` = ?");
	        PreparedStatement ps2 = sql.con.prepareStatement("INSERT INTO `k_tags` (`ID`, `key`, `value`) VALUES (NULL, ?, ?);");
